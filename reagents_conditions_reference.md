# Reagents & Conditions Reference — All Reactions

Generated from the app's own `POOL` data (115 reactions across 9 topics), for review.

**How grading works, so the example answers below make sense:**
- A submitted answer must exactly match ONE complete accepted combination — you can't mix a reagent from one combo with a condition from another.
- Matching is case-insensitive.
- **"heat" and "heat under reflux" are treated as exactly the same condition** — reflux is lab technique (how the heating is done so volatiles don't escape), not a separately markable point in H2 exams. Every reaction below is written as just "heat"; typing "heat under reflux" instead is silently accepted as equivalent, never penalized.
- If a combo doesn't call out any temperature/condition at all, adding "room temperature" to your answer is not penalized (it's treated as the implicit default). If a combo does specify a temperature (even a mild one), you need to give it.
- "Also accepted" lines below are fully independent alternative combos (e.g. AlCl3 OR FeCl3 OR Fe OR Al as catalyst) — not something you mix and match with the primary answer.
- Some questions may accept additional combos beyond what's listed here, if a different POOL reaction produces the exact same product from the same starting material (resolved at runtime, not listed statically here).

---

## Alkanes

### Free radical substitution (Cl2)
- **Reagents/Conditions shown to the student:** Cl2(g), UV light
- **Primary correct answer:** `Cl2(g), UV light`

### Free radical substitution (Br2)
- **Reagents/Conditions shown to the student:** Br2(g), UV light
- **Primary correct answer:** `Br2(g), UV light`

### Combustion of alkanes
- **Reagents/Conditions shown to the student:** Combustion in air
- **Primary correct answer:** `Combustion in air`

## Alkenes

### Electrophilic addition of Br2
- **Reagents/Conditions shown to the student:** Br2 in CCl4, room temperature, in the dark
- **Primary correct answer:** `Br2 in CCl4, room temperature, in the dark`

### Electrophilic addition of Cl2
- **Reagents/Conditions shown to the student:** Cl2 in CCl4, room temperature, in the dark
- **Primary correct answer:** `Cl2 in CCl4, room temperature, in the dark`

### Catalytic hydrogenation (H2 / Ni)
- **Reagents/Conditions shown to the student:** H2(g), Ni catalyst
- **Primary correct answer:** `H2(g), Ni catalyst`

### Mild oxidation to diol
- **Reagents/Conditions shown to the student:** KMnO4(aq), NaOH(aq), cold
- **Primary correct answer:** `KMnO4(aq), NaOH(aq), cold`

### Oxidative cleavage (hot KMnO4)
- **Reagents/Conditions shown to the student:** KMnO4(aq), H2SO4(aq), heat
- **Primary correct answer:** `KMnO4(aq), H2SO4(aq), heat`

### Electrophilic addition of HBr
- **Reagents/Conditions shown to the student:** HBr(g), room temperature
- **Primary correct answer:** `HBr(g), room temperature`

### Electrophilic addition of HCl
- **Reagents/Conditions shown to the student:** HCl(g), room temperature
- **Primary correct answer:** `HCl(g), room temperature`

### Electrophilic addition of steam
- **Reagents/Conditions shown to the student:** H2O(g), H3PO4 catalyst, heat, high pressure
- **Primary correct answer:** `H2O(g), H3PO4 catalyst, heat, high pressure`
- **Also accepted:** `H2SO4(aq), then H2O`

### Formation of a bromohydrin
- **Reagents/Conditions shown to the student:** Br2(aq), room temperature
- **Primary correct answer:** `Br2(aq), room temperature`

## Arenes

### Chlorination of benzene
- **Reagents/Conditions shown to the student:** Cl2(g), AlCl3 (or FeCl3/Fe/Al), warm
- **Primary correct answer:** `Cl2(g), AlCl3, warm`
- **Also accepted:** `Cl2(g), FeCl3, warm`
- **Also accepted:** `Cl2(g), Fe, warm`
- **Also accepted:** `Cl2(g), Al, warm`

### Bromination of benzene
- **Reagents/Conditions shown to the student:** Br2(l), AlBr3 (or FeBr3/Fe/Al), warm
- **Primary correct answer:** `Br2(l), AlBr3, warm`
- **Also accepted:** `Br2(l), FeBr3, warm`
- **Also accepted:** `Br2(l), Fe, warm`
- **Also accepted:** `Br2(l), Al, warm`

### Halogenation of benzene (no catalyst)
- **Reagents/Conditions shown to the student:** Cl2(g) or Br2(l), no Lewis acid catalyst
- **Primary correct answer:** `Cl2(g), no Lewis acid catalyst`
- **Also accepted:** `Br2(l), no Lewis acid catalyst`

### Nitration of benzene
- **Reagents/Conditions shown to the student:** conc HNO3, conc H2SO4, 50°C
- **Primary correct answer:** `conc HNO3, conc H2SO4, 50°C`

### Nitration of benzene (no H2SO4)
- **Reagents/Conditions shown to the student:** conc HNO3 only, no conc H2SO4
- **Primary correct answer:** `conc HNO3 only, no conc H2SO4`

### Friedel-Crafts alkylation of benzene
- **Reagents/Conditions shown to the student:** AlCl3, warm
- **Primary correct answer:** `AlCl3, warm`

### Benzene + bromine in tetrachloromethane
- **Reagents/Conditions shown to the student:** Br2 in CCl4
- **Primary correct answer:** `Br2 in CCl4`

### Benzene + aqueous bromine
- **Reagents/Conditions shown to the student:** Br2(aq)
- **Primary correct answer:** `Br2(aq)`

### Ring chlorination of a substituted benzene
- **Reagents/Conditions shown to the student:** Cl2(g), AlCl3, room temperature, absence of UV
- **Primary correct answer:** `Cl2(g), AlCl3, room temperature, absence of UV`

### Ring bromination of a substituted benzene
- **Reagents/Conditions shown to the student:** Br2(l), AlBr3, room temperature, absence of UV
- **Primary correct answer:** `Br2(l), AlBr3, room temperature, absence of UV`

### Ring nitration of a substituted benzene
- **Reagents/Conditions shown to the student:** conc HNO3, conc H2SO4, 30°C
- **Primary correct answer:** `conc HNO3, conc H2SO4, 30°C`

### Friedel-Crafts alkylation of a substituted benzene
- **Reagents/Conditions shown to the student:** AlCl3, room temperature
- **Primary correct answer:** `AlCl3, room temperature`

### Side-chain chlorination of alkylbenzene
- **Reagents/Conditions shown to the student:** Cl2(g), UV light
- **Primary correct answer:** `Cl2(g), UV light`

### Side-chain bromination of alkylbenzene
- **Reagents/Conditions shown to the student:** Br2(g), UV light
- **Primary correct answer:** `Br2(g), UV light`

### Side-chain oxidation of alkylbenzene (acidic KMnO4)
- **Reagents/Conditions shown to the student:** KMnO4(aq), H2SO4(aq), heat
- **Primary correct answer:** `KMnO4(aq), H2SO4(aq), heat`

### Side-chain oxidation of alkylbenzene (alkaline KMnO4)
- **Reagents/Conditions shown to the student:** KMnO4(aq), NaOH(aq), heat; then H2SO4(aq)
- **Primary correct answer:** `KMnO4(aq), NaOH(aq), heat, H2SO4(aq)`

### Side-chain oxidation of tert-butylbenzene
- **Reagents/Conditions shown to the student:** KMnO4(aq), H2SO4(aq), heat
- **Primary correct answer:** `KMnO4(aq), H2SO4(aq), heat`

## Halogen Derivatives

### Hydrolysis of halogenoalkanes
- **Reagents/Conditions shown to the student:** NaOH(aq), heat
- **Primary correct answer:** `NaOH(aq), heat`

### Formation of nitriles
- **Reagents/Conditions shown to the student:** KCN in ethanol, heat
- **Primary correct answer:** `KCN in ethanol, heat`

### Acidic hydrolysis of nitriles
- **Reagents/Conditions shown to the student:** H2SO4(aq), heat
- **Primary correct answer:** `H2SO4(aq), heat`

### Alkaline hydrolysis of nitriles
- **Reagents/Conditions shown to the student:** NaOH(aq), heat
- **Primary correct answer:** `NaOH(aq), heat`

### Reduction of nitriles
- **Reagents/Conditions shown to the student:** LiAlH4 in dry ether (or H2(g), Ni catalyst)
- **Primary correct answer:** `LiAlH4, dry ether`
- **Also accepted:** `H2(g), Ni catalyst`

### Formation of primary amines
- **Reagents/Conditions shown to the student:** NH3 in ethanol, heated under high pressure
- **Primary correct answer:** `NH3 in ethanol, heated under high pressure`

### Elimination of hydrogen halide
- **Reagents/Conditions shown to the student:** NaOH in ethanol, heat
- **Primary correct answer:** `NaOH in ethanol, heat`

### Nucleophilic substitution of fluoroalkanes
- **Reagents/Conditions shown to the student:** NaOH(aq), heat
- **Primary correct answer:** `NaOH(aq), heat`

### Nucleophilic substitution of halogenoarenes
- **Reagents/Conditions shown to the student:** NaOH(aq), heat (halogenoalkane conditions)
- **Primary correct answer:** `NaOH(aq), heat`

### Electrophilic substitution of halogenoarenes
- **Reagents/Conditions shown to the student:** conc HNO3, conc H2SO4 (occurs less readily than benzene)
- **Primary correct answer:** `conc HNO3, conc H2SO4`

## Amines

### Formation of secondary amines
- **Reagents/Conditions shown to the student:** heat
- **Primary correct answer:** `heat`

### Formation of tertiary amines
- **Reagents/Conditions shown to the student:** heat
- **Primary correct answer:** `heat`

### Formation of quaternary ammonium salts
- **Reagents/Conditions shown to the student:** excess haloalkane, heat
- **Primary correct answer:** `excess haloalkane, heat`

## Hydroxy Compounds

### Combustion of alcohols
- **Reagents/Conditions shown to the student:** Combustion in air
- **Primary correct answer:** `Combustion in air`

### Reaction of alcohols with sodium
- **Reagents/Conditions shown to the student:** Na(s), room temperature
- **Primary correct answer:** `Na(s), room temperature`

### Reaction of phenols with sodium
- **Reagents/Conditions shown to the student:** Na(s), room temperature
- **Primary correct answer:** `Na(s), room temperature`

### Reaction of phenols with sodium hydroxide
- **Reagents/Conditions shown to the student:** NaOH(aq), room temperature
- **Primary correct answer:** `NaOH(aq), room temperature`

### Reaction of phenols with carbonates
- **Reagents/Conditions shown to the student:** Na2CO3(aq)
- **Primary correct answer:** `Na2CO3(aq)`

### Reaction of alcohols with sodium hydroxide
- **Reagents/Conditions shown to the student:** NaOH(aq)
- **Primary correct answer:** `NaOH(aq)`

### Reaction of alcohols with carbonates
- **Reagents/Conditions shown to the student:** Na2CO3(aq)
- **Primary correct answer:** `Na2CO3(aq)`

### Conversion of alcohols to chloroalkanes (PCl5)
- **Reagents/Conditions shown to the student:** PCl5, room temperature
- **Primary correct answer:** `PCl5, room temperature`

### Conversion of tertiary alcohols to chloroalkanes (HCl)
- **Reagents/Conditions shown to the student:** conc HCl, room temperature
- **Primary correct answer:** `conc HCl, room temperature`

### Conversion of primary/secondary alcohols to chloroalkanes (HCl/ZnCl2)
- **Reagents/Conditions shown to the student:** conc HCl, ZnCl2, heat
- **Primary correct answer:** `conc HCl, ZnCl2, heat`

### Conversion of alcohols to bromoalkanes
- **Reagents/Conditions shown to the student:** conc HBr (or NaBr, conc H2SO4, heat)
- **Primary correct answer:** `conc HBr`
- **Also accepted:** `NaBr, conc H2SO4, heat`

### Conversion of alcohols to iodoalkanes
- **Reagents/Conditions shown to the student:** conc HI
- **Primary correct answer:** `conc HI`

### Conversion of phenols to halogenoarenes
- **Reagents/Conditions shown to the student:** Halogenating reagents (requires extremely harsh conditions)
- **Primary correct answer:** `Halogenating reagents`

### Dehydration of alcohols
- **Reagents/Conditions shown to the student:** conc H3PO4 catalyst, heat
- **Primary correct answer:** `conc H3PO4 catalyst, heat`
- **Also accepted:** `conc H2SO4, heat`

### Dehydration of phenols
- **Reagents/Conditions shown to the student:** conc H3PO4 catalyst, heat
- **Primary correct answer:** `conc H3PO4 catalyst, heat`
- **Also accepted:** `conc H2SO4, heat`

### Condensation of alcohols with carboxylic acids
- **Reagents/Conditions shown to the student:** few drops conc H2SO4, heat
- **Primary correct answer:** `few drops conc H2SO4, heat`

### Condensation of phenols with carboxylic acids
- **Reagents/Conditions shown to the student:** few drops conc H2SO4, heat
- **Primary correct answer:** `few drops conc H2SO4, heat`

### Condensation of alcohols with acyl chlorides
- **Reagents/Conditions shown to the student:** room temperature
- **Primary correct answer:** `room temperature`

### Condensation of phenols with acyl chlorides
- **Reagents/Conditions shown to the student:** room temperature
- **Primary correct answer:** `room temperature`

### Oxidation of primary alcohols to aldehydes
- **Reagents/Conditions shown to the student:** K2Cr2O7(aq)/H2SO4(aq), heat with immediate distillation
- **Primary correct answer:** `K2Cr2O7(aq), H2SO4(aq), heat with immediate distillation`
- **Also accepted:** `KMnO4(aq), H2SO4(aq), heat with immediate distillation`

### Oxidation of primary alcohols to carboxylic acids
- **Reagents/Conditions shown to the student:** K2Cr2O7(aq)/H2SO4(aq) (or KMnO4(aq)/H2SO4(aq)), heat
- **Primary correct answer:** `K2Cr2O7(aq), H2SO4(aq), heat`
- **Also accepted:** `KMnO4(aq), H2SO4(aq), heat`

### Oxidation of secondary alcohols to ketones
- **Reagents/Conditions shown to the student:** K2Cr2O7(aq)/H2SO4(aq) (or KMnO4(aq)/H2SO4(aq)), heat
- **Primary correct answer:** `K2Cr2O7(aq), H2SO4(aq), heat`
- **Also accepted:** `KMnO4(aq), H2SO4(aq), heat`

### Oxidation of tertiary alcohols
- **Reagents/Conditions shown to the student:** K2Cr2O7(aq)/H2SO4(aq) (or KMnO4(aq)/H2SO4(aq)), heat
- **Primary correct answer:** `K2Cr2O7(aq), H2SO4(aq), heat`
- **Also accepted:** `KMnO4(aq), H2SO4(aq), heat`

### Oxidation of phenols
- **Reagents/Conditions shown to the student:** K2Cr2O7(aq)/H2SO4(aq) (or KMnO4(aq)/H2SO4(aq)), heat
- **Primary correct answer:** `K2Cr2O7(aq), H2SO4(aq), heat`
- **Also accepted:** `KMnO4(aq), H2SO4(aq), heat`

### Nitration of phenol
- **Reagents/Conditions shown to the student:** HNO3(aq)
- **Primary correct answer:** `HNO3(aq)`

### Monobromination of phenol
- **Reagents/Conditions shown to the student:** Br2(l), or Br2 in CCl4
- **Primary correct answer:** `Br2(l)`
- **Also accepted:** `Br2 in CCl4`

### Bromination of phenol with aqueous bromine
- **Reagents/Conditions shown to the student:** Br2(aq), excess, room temperature
- **Primary correct answer:** `Br2(aq), excess, room temperature`

### Trinitration of phenol (picric acid)
- **Reagents/Conditions shown to the student:** conc HNO3, room temperature
- **Primary correct answer:** `conc HNO3, room temperature`

### Tri-iodomethane test (CH3CH(OH)- alcohol)
- **Reagents/Conditions shown to the student:** I2(aq), NaOH(aq), warm
- **Primary correct answer:** `I2(aq), NaOH(aq), warm`

## Carbonyl Compounds

### Nucleophilic addition of hydrogen cyanide
- **Reagents/Conditions shown to the student:** HCN, trace KCN catalyst, cold
- **Primary correct answer:** `HCN, trace KCN catalyst, cold`

### Reduction of aldehyde to primary alcohol
- **Reagents/Conditions shown to the student:** LiAlH4, dry ether, room temperature (or H2(g), Ni catalyst)
- **Primary correct answer:** `LiAlH4, dry ether, room temperature`
- **Also accepted:** `H2(g), Ni catalyst`

### Reduction of ketone to secondary alcohol
- **Reagents/Conditions shown to the student:** LiAlH4, dry ether, room temperature (or H2(g), Ni catalyst)
- **Primary correct answer:** `LiAlH4, dry ether, room temperature`
- **Also accepted:** `H2(g), Ni catalyst`

### Iodoform test (methyl ketone)
- **Reagents/Conditions shown to the student:** I2(aq), NaOH(aq), warm
- **Primary correct answer:** `I2(aq), NaOH(aq), warm`

### Tri-iodomethane test (ethanal)
- **Reagents/Conditions shown to the student:** I2(aq), NaOH(aq), warm
- **Primary correct answer:** `I2(aq), NaOH(aq), warm`

### Oxidation of aldehyde to carboxylic acid
- **Reagents/Conditions shown to the student:** K2Cr2O7(aq) (or KMnO4(aq)), H2SO4(aq), heat
- **Primary correct answer:** `K2Cr2O7(aq), H2SO4(aq), heat`
- **Also accepted:** `KMnO4(aq), H2SO4(aq), heat`

### Oxidation of ketone with acidified oxidising agents
- **Reagents/Conditions shown to the student:** K2Cr2O7(aq) (or KMnO4(aq)), H2SO4(aq), heat
- **Primary correct answer:** `K2Cr2O7(aq), H2SO4(aq), heat`
- **Also accepted:** `KMnO4(aq), H2SO4(aq), heat`

### Tollens' reagent test (aldehyde)
- **Reagents/Conditions shown to the student:** Tollens' reagent, warm
- **Primary correct answer:** `Tollens' reagent, warm`

### Tollens' reagent test (ketone)
- **Reagents/Conditions shown to the student:** Tollens' reagent, warm
- **Primary correct answer:** `Tollens' reagent, warm`

### Fehling's reagent test (aliphatic aldehyde)
- **Reagents/Conditions shown to the student:** Fehling's reagent, warm
- **Primary correct answer:** `Fehling's reagent, warm`

### Fehling's reagent test (ketone)
- **Reagents/Conditions shown to the student:** Fehling's reagent, warm
- **Primary correct answer:** `Fehling's reagent, warm`

### Fehling's reagent test (benzaldehyde)
- **Reagents/Conditions shown to the student:** Fehling's reagent, warm
- **Primary correct answer:** `Fehling's reagent, warm`

## Carboxylic Acids & Derivatives

### Reaction of carboxylic acid with metal
- **Reagents/Conditions shown to the student:** Na(s)
- **Primary correct answer:** `Na(s)`

### Reaction of carboxylic acid with alkali
- **Reagents/Conditions shown to the student:** NaOH(aq)
- **Primary correct answer:** `NaOH(aq)`

### Reaction of carboxylic acid with carbonate
- **Reagents/Conditions shown to the student:** Na2CO3(aq)
- **Primary correct answer:** `Na2CO3(aq)`

### Reaction of carboxylic acid with hydrogencarbonate
- **Reagents/Conditions shown to the student:** NaHCO3(aq)
- **Primary correct answer:** `NaHCO3(aq)`

### Formation of acyl chloride using PCl5
- **Reagents/Conditions shown to the student:** PCl5, room temperature
- **Primary correct answer:** `PCl5, room temperature`

### Formation of acyl chloride using PCl3
- **Reagents/Conditions shown to the student:** PCl3
- **Primary correct answer:** `PCl3`

### Formation of acyl chloride using SOCl2
- **Reagents/Conditions shown to the student:** SOCl2, warm
- **Primary correct answer:** `SOCl2, warm`

### Reduction of carboxylic acid to primary alcohol
- **Reagents/Conditions shown to the student:** LiAlH4 in dry ether
- **Primary correct answer:** `LiAlH4, dry ether`

### Reduction of carboxylic acid with NaBH4
- **Reagents/Conditions shown to the student:** NaBH4
- **Primary correct answer:** `NaBH4`

### Reduction of carboxylic acid with H2/Ni
- **Reagents/Conditions shown to the student:** H2(g), Ni catalyst
- **Primary correct answer:** `H2(g), Ni catalyst`

### Oxidation of methanoic acid and ethanedioic acid
- **Reagents/Conditions shown to the student:** KMnO4(aq)/H2SO4(aq), heat
- **Primary correct answer:** `KMnO4(aq), H2SO4(aq), heat`

### Hydrolysis of acyl chloride
- **Reagents/Conditions shown to the student:** H2O, room temperature
- **Primary correct answer:** `H2O, room temperature`

### Reaction of acyl chloride with ammonia
- **Reagents/Conditions shown to the student:** excess NH3(aq)
- **Primary correct answer:** `excess NH3(aq)`

### Reaction of acyl chloride with primary amine
- **Reagents/Conditions shown to the student:** excess primary amine
- **Primary correct answer:** `excess primary amine`

### Reaction of acyl chloride with secondary amine
- **Reagents/Conditions shown to the student:** excess secondary amine
- **Primary correct answer:** `excess secondary amine`

### Acid hydrolysis of ester
- **Reagents/Conditions shown to the student:** dilute acid, heat
- **Primary correct answer:** `dilute acid, heat`
- **Also accepted:** `HCl(aq), heat`
- **Also accepted:** `H2SO4(aq), heat`

### Alkaline hydrolysis of ester
- **Reagents/Conditions shown to the student:** NaOH(aq), heat
- **Primary correct answer:** `NaOH(aq), heat`

### Hydrolysis of acid anhydride
- **Reagents/Conditions shown to the student:** H2O
- **Primary correct answer:** `H2O`

### Acid hydrolysis of amide
- **Reagents/Conditions shown to the student:** dilute acid, heat
- **Primary correct answer:** `dilute acid, heat`
- **Also accepted:** `HCl(aq), heat`
- **Also accepted:** `H2SO4(aq), heat`

### Alkaline hydrolysis of amide
- **Reagents/Conditions shown to the student:** NaOH(aq), heat
- **Primary correct answer:** `NaOH(aq), heat`

### Reduction of amide
- **Reagents/Conditions shown to the student:** LiAlH4, dry ether
- **Primary correct answer:** `LiAlH4, dry ether`

## Nitrogen Compounds

### Diazotisation of phenylamine
- **Reagents/Conditions shown to the student:** NaNO2(aq)/HCl(aq), 0-10°C
- **Primary correct answer:** `NaNO2(aq), HCl(aq), 0-10°C`

### Azo coupling with phenol
- **Reagents/Conditions shown to the student:** phenol, alkaline solution, 0-10°C
- **Primary correct answer:** `phenol, alkaline solution, 0-10°C`

### Azo coupling with phenylamine
- **Reagents/Conditions shown to the student:** phenylamine, 0-10°C
- **Primary correct answer:** `phenylamine, 0-10°C`

### Reduction of nitrobenzene to phenylamine
- **Reagents/Conditions shown to the student:** Sn(s)/conc HCl, heat, then NaOH(aq) (or H2(g)/Ni catalyst)
- **Primary correct answer:** `Sn(s), conc HCl, heat, NaOH(aq)`
- **Also accepted:** `H2(g), Ni catalyst`

### Formation of phenylammonium salt
- **Reagents/Conditions shown to the student:** dilute acid
- **Primary correct answer:** `dilute acid`
- **Also accepted:** `HCl(aq)`
- **Also accepted:** `H2SO4(aq)`

### Bromination of phenylamine
- **Reagents/Conditions shown to the student:** Br2(aq), room temperature
- **Primary correct answer:** `Br2(aq), room temperature`

### Reaction of amino acids with acid
- **Reagents/Conditions shown to the student:** HCl(aq)
- **Primary correct answer:** `HCl(aq)`

### Reaction of amino acids with alkali
- **Reagents/Conditions shown to the student:** NaOH(aq)
- **Primary correct answer:** `NaOH(aq)`

### Formation of a peptide bond
- **Reagents/Conditions shown to the student:** condensation (enzyme catalysed in vivo)
- **Primary correct answer:** `condensation, enzyme catalyst`

### Acid hydrolysis of a peptide bond
- **Reagents/Conditions shown to the student:** H2SO4(aq), heat
- **Primary correct answer:** `H2SO4(aq), heat`

### Alkaline hydrolysis of a peptide bond
- **Reagents/Conditions shown to the student:** NaOH(aq), heat
- **Primary correct answer:** `NaOH(aq), heat`
